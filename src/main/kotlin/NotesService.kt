object NotesService {
    ...
    fun addComment(id: Int, comment: Comment): Comment //Создает новый комментарий у текущего пользователя.
    {
    ...
    @Throws(NoteNotFoundException::class)
    fun deleteNote(id: Int): Boolean //Удаляет заметку текущего пользователя по id.
    {
        if (id in notes.indices) {
            notes.removeAt(id)
        } else {
    ...
    @Throws(NoteNotFoundException::class)
    fun editNote(id: Int, note: Note) //Редактирует заметку текущего пользователя.
    {
        if (id in notes.indices) {
            notes[id] = note
        } else {
    ...
    // реализована как функция addComment
//    fun createComment(id: Int, comment: Comment) //Добавляет новый комментарий к заметке.
//    {
//
//    }
    ...
    fun editComment(id: Int, idComment: Int, text: String): Boolean //Редактирует указанный комментарий у заметки.
    {
        if (!notes[id].comments.isNullOrEmpty()) {
            notes[id].comments!!.forEach {
                if (it.id == idComment) {
    ...
    @Throws(NoteNotFoundException::class)
    fun getComments(id: Int): ArrayList<Comment> //Возвращает список комментариев к заметке.
    {
